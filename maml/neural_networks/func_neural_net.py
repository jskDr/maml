...
def back_propagate(para_input):
    ...
    pool_index, train_start, train_end, learn_rate, rms_decay, n_epochs, \
        batch_size, n_cores, n_hist, n_check, threshold = para_input
    hist_count = 0
...
def train(X_train,X_test,Y_train,Y_test,nneurons,input_act_funcs,learn_rate=0.001,rms_decay=0.9,n_epochs=10000,
    batch_size=256,n_cores=1,n_hist=20,n_check=50,threshold=0.1):
    """
    ...
    para_inputs = [[i, j, j+big_batch_size,learn_rate,rms_decay,n_epochs,batch_size,n_cores,n_hist,
        n_check,threshold] for i,j in enumerate(range(0,N,big_batch_size))]
...
if __name__ == '__main__':
    ...
    Y_train = Y[:N/2]
    Y_test = Y[N/2:]
    nnet = train(X_train,X_test,Y_train,Y_test,[20,10],'tanh',learn_rate=0.001,n_epochs=50000,n_cores=20)
